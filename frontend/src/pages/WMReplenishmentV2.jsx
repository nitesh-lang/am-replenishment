...
import {
  Search, Download, ChevronDown, ChevronRight, Settings2, RotateCcw,
} from "lucide-react";
...
/* ============================================================
   CLICKTECH (WM) REPLENISHMENT V2 — TanStack + v2 pattern
   Endpoints reused:
     GET  /api/wm-replenishment/?cover_weeks&from_week&to_week
     POST /api/wm-replenishment/save  { model, po_requirement, remarks }
     POST /api/wm-replenishment/reset
============================================================ */

const BASE = import.meta.env.DEV ? (import.meta.env.VITE_API_BASE || "http://localhost:8060") : "";

export default function WMReplenishmentV2() {
  ...
  /* In-flight save indicator per model */
  ...
  useEffect(() => { setPage(1); }, [search, view, category, fromWeek, toWeek]);

  /* ============================================================
     SAVE ROW (po_requirement + remarks)
  ============================================================ */
  async function saveRow(model, po_requirement, remarks) {
    if (!model) return;
    setSavingMap(p => ({ ...p, [model]: "saving" }));
    try {
      const res = await fetch(`${BASE}/api/wm-replenishment/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model,
          po_requirement: parseInt(po_requirement) || 0,
          remarks: remarks ?? "",
        }),
      });
      const j = await res.json();
      setSavingMap(p => ({ ...p, [model]: j.status === "saved" ? "saved" : "error" }));
      if (j.status === "saved") logUsage("save", "wm-replenishment", { model });
  ...
  async function resetAll() {
    if (!confirm("Reset all Clicktech (WM) po_requirement + remarks?")) return;
    try {
      ...
      if (fromWeek) params.append("from_week", fromWeek);
      if (toWeek)   params.append("to_week",   toWeek);
      const res = await fetch(`${BASE}/api/wm-replenishment/?${params}`).then(r => r.json());
      setRows(res.data || []);
    } catch {}
  }
  ...
  const categories = useMemo(
    () => ["All", ...[...new Set(rows.map(r => r.category).filter(Boolean))].sort()],
    [rows]
  );
  ...
  const views = useMemo(() => [
    ...
    { key: "hazmat",     label: "Hazmat only",    count: rows.filter(r => { const h = (r.hazmat_type || "").toLowerCase(); return h && !h.includes("non"); }).length },
  ], [rows]);
  ...
  const columns = useMemo(() => [
    ...
    { id: "category",         accessorKey: "category",         header: "Category",     size: 110, meta: { group: "id" } },
    { id: "hazmat_type",      accessorKey: "hazmat_type",      header: "Hazmat",       size: 90,  meta: { group: "id" } },
    ...
    { id: "po_requirement",   accessorKey: "po_requirement",   header: "PO Req",       size: 100, meta: { group: "plan", editable: true } },
    { id: "remarks",          accessorKey: "remarks",          header: "Remarks",      size: 200, meta: { group: "edit", editable: true } },
  ], []);
  ...
  function exportCSV() {
    ...
    a.download = `wm_replenishment_${category.toLowerCase()}_${view}.csv`;
    a.click();
    logUsage("export", "wm-replenishment", { rows: filteredRows.length });
  }
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <div className="w-full px-6 pt-5 pb-10">
        ...
        <div className="flex items-end justify-between mb-4">
          <div>
            ...
            <h1 className="text-2xl font-semibold mt-0.5 tracking-tight">
              Clicktech Replenishment <span className="text-slate-300 font-normal">·</span> <span className="text-slate-500 text-lg font-medium">{category}</span>
            </h1>
          </div>
        ...
        <div className="grid grid-cols-5 gap-3 mb-5">
          <KPICard label="Total Models"     value={rows.length}            hint={`${categories.length - 1} categories`} />
        ...
        <div className="mb-3 grid grid-cols-12 gap-3">
          ...
          <div className="col-span-3">
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm px-3 py-2 flex items-center gap-2">
              <span className="text-[11px] uppercase tracking-wider text-slate-500 font-semibold whitespace-nowrap">Category</span>
              <select
                value={category}
                ...
                {table.getRowModel().rows.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE).map((trow, ri) => {
                  ...
                  return (
                    <React.Fragment key={r.model || globalRowIdx}>
                      ...
                      >
                        {trow.getVisibleCells().map((cell, ci) => {
                          ...
                          if (colId === "model") {
                            ...
                            content = <span className="text-slate-600">{r.category || "—"}</span>;
                      ...
                      {isExpanded && (
                        <WMDetailRow row={r} colSpan={columns.length} />
                      )}
                    </React.Fragment>
                  );
                })}
        ...
        <div className="mt-4 flex flex-wrap gap-5 text-xs text-slate-500 px-1">
          ...
          <Legend swatch="bg-amber-100"  label="Hazmat" />
...
function WMDetailRow({ row, colSpan }) {
  const cbSales  = Number(row.cb_3m_sales || 0);
  const amzSales = Number(row.amazon_3m_sales || 0);
  const total    = cbSales + amzSales;
  const cbPct    = total > 0 ? Math.round((cbSales / total) * 100) : 0;
  const amzPct   = 100 - cbPct;
        ...
        <div className="px-6 py-5 bg-gradient-to-b from-indigo-50/40 to-white border-l-4 border-indigo-500 grid grid-cols-12 gap-5">
          <div className="col-span-4">
            ...
            <div className="bg-white rounded-md border border-slate-200 p-3 space-y-2.5 text-xs">
              ...
              <div>
                <div className="flex justify-between mb-1"><span>Amazon</span><span className="font-mono tabular-nums font-semibold">{Math.round(amzSales)} ({amzPct}%)</span></div>
                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${amzPct}%` }} /></div>
              </div>
          ...
          <div className="col-span-4">
            ...
            <div className="bg-white rounded-md border border-slate-200 p-3 space-y-2 text-xs">
              ...
              <KV label="PO Req"      val={Math.round(row.po_requirement || 0)} accent />
              <KV label="Hazmat"      val={row.hazmat_type || "—"} />
            </div>
          </div>
        </div>
      </td>
